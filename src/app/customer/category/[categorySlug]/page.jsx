...
const CategoryPage = () => {
  ...
  }, [categorySlug, currentPage, sortBy, sortOrder, selectedSubcategory]);

  const fetchCategoryData = async () => {
    try {
      setLoading(true);
      
      // Construct API URL for category by slug
      const apiUrl = `${process.env.NEXT_PUBLIC_API_URL}/api/categories/slug/${categorySlug}`;
      ...
      if (data.success) {
        setCategoryInfo(data.data.category);
      ...
      <div className="px-8 py-16">
        ...
        {subcategories.length > 0 && (
          <div className="flex justify-center mb-16">
            <div className="bg-white border border-stone-200 inline-flex flex-wrap">
              <button
                onClick={() => handleSubcategoryFilter(null)}
                className={`px-6 py-3 font-light tracking-wide text-sm transition-colors ${
                  !selectedSubcategory 
        ...
        {/* Quick Links to Subcategories */}
        {subcategories.length > 0 && (
          <div className="mt-20 pt-16 border-t border-stone-200">
            <div className="text-center mb-12">
              <h2 className="text-2xl font-light text-stone-800 tracking-widest uppercase mb-4">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
              {subcategories.map((subcategory) => (
                ...
                >
                  ...
                  <div className="w-8 h-px bg-stone-300 mx-auto mt-4 group-hover:bg-stone-400 transition-colors"></div>
                </Link>
              ))}
            </div>
          </div>
